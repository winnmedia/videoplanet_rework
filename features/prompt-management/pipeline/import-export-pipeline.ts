...
import { VideoPlanetPrompt, VideoPlanetPromptSchema, parsePrompt, validatePrompt } from '../../../entities/prompt'

interface PromptExportPackage {
...
// Mock adapters
class OpenAiAdapter {
  convertToVideoPlanet(data: any, options?: any): VideoPlanetPrompt {
    return data as VideoPlanetPrompt
  }
}
...
export class ImportExportPipeline {
        ...
        return {
          ...
          data: {
            ...
            errors: integrityReport.errors.map(e => ({
              error: e.message,
              severity: e.severity === 'critical' ? 'high' : e.severity as 'low' | 'medium' | 'high'
            }))
          },
          integrityReport,
          processingTime: performance.now() - startTime
  ...
  private async executeJsonExport(job: ExportJob): Promise<ExportResult['data']> {
    ...
    try {
      exportPackageSchema.parse(exportPackage)
    } catch (error) {
      if (error instanceof z.ZodError) {
        throw new Error(`Export package validation failed: ${error.errors.map(e => e.message).join(', ')}`)
      }
      throw error
    }
  ...
  private async validateDataIntegrity(sourceData: any, sourceFormat: string): Promise<DataIntegrityReport> {
    ...
    try {
      if (sourceFormat === 'videoplanet') {
        ...
        const validationResult = validationSchema.safeParse({ prompts: sourceData.prompts || [sourceData] })
        ...
        if (!validationResult.success) {
          ...
          statistics.validRecords = statistics.totalRecords - validationResult.error.errors.length
          statistics.invalidRecords = validationResult.error.errors.length
        } else {
          statistics.validRecords = statistics.totalRecords
  ...
  private async importSinglePrompt(
    ...
    try {
      ...
      const promptSchema = z.object({
        ...
        version: z.string(),
        metadata: z.any(),
        promptStructure: z.any().optional(),
        generationSettings: z.any().optional(),
        qualityAssurance: z.any().optional(),
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const prompt: VideoPlanetPrompt = {
        ...
        metadata: {
          ...
          category: (row.category as any) || 'storyboard',
          tags: row.tags ? row.tags.split(';') : [],
          difficulty: (row.difficulty as any) || 'medium',
          estimatedTokens: parseInt(row.estimatedTokens) || 100,
          language: row.language || 'ko',
          targetAudience: (row.targetAudience as any) || 'professional'
        },
        promptStructure: {
          shotBreakdown: [{
            shotNumber: 1,
            description: row.description || 'Default shot from CSV import',
            cameraAngle: 'medium' as any,
            duration: 5,
            visualElements: [],
            generationPrompt: row.prompt || 'Default generation prompt'
          }],
          styleGuide: {
            artStyle: (row.artStyle as any) || 'photorealistic',
            colorPalette: (row.colorPalette as any) || 'natural',
            visualMood: (row.visualMood as any) || 'happy'
          }
        },
        generationSettings: {
          provider: (row.provider as any) || 'google',
          model: row.model || 'imagen-4.0-fast-generate-preview-06-06',
          parameters: {
            aspectRatio: (row.aspectRatio as any) || '16:9',
            quality: (row.quality as any) || 'high',
            stylization: parseFloat(row.stylization) || 0.8,
            coherence: parseFloat(row.coherence) || 0.9
          }
        },
        qualityAssurance: {
          validationRules: {
            minConsistencyScore: parseFloat(row.minConsistencyScore) || 0.7,
            maxRegenerationCount: parseInt(row.maxRegenerationCount) || 3,
            requiredElements: [],
            forbiddenElements: []
          }
        },
        usage: {
          createdBy: row.createdBy || 'csv_import',
          createdAt: new Date().toISOString(),
          usageCount: parseInt(row.usageCount) || 0,
          totalGenerations: parseInt(row.totalGenerations) || 0
        },
