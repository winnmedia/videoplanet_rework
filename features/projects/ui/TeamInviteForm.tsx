...
import { useState, KeyboardEvent, useEffect, useCallback, useMemo } from 'react'
...
export function TeamInviteForm({ 
  ...
  // 쿨다운 툴팁 메시지 메모화
  const cooldownTooltipMessage = useMemo(() => {
    if (cooldownRemaining <= 0) return null
    const seconds = Math.ceil(cooldownRemaining / 1000)
    return `${seconds}초 후 재전송 가능합니다`
  }, [cooldownRemaining])

  // 쿨다운 타이머 (useCallback으로 최적화)
  ...
  useEffect(() => {
    ...
    if (remaining && remaining > 0) {
      const timer = setInterval(() => {
        const newRemaining = updateCooldown()
        if (newRemaining === 0 || newRemaining === undefined) {
          clearInterval(timer)
        }
      }, 1000)
      ...
      {!canInvite ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end pt-4 border-t border-gray-200">
            <Tooltip 
              content={cooldownTooltipMessage || ''} 
              disabled={!cooldownTooltipMessage}
              position="top"
            >
              <Button
                type="submit"
                ...
                className="min-w-32"
                aria-label={cooldownRemaining > 0 ? `초대 보내기 - ${cooldownTooltipMessage}` : '초대 보내기'}
                aria-describedby={cooldownRemaining > 0 ? 'cooldown-info' : undefined}
              >
                {isInviting ? (
                  <div className="flex items-center">
                  ...
                  <div className="flex items-center">
                    <Clock className="w-4 h-4 mr-2" />
                    {Math.ceil(cooldownRemaining / 1000)}초 대기
                  </div>
                ) : (
                  '초대 보내기'
                )}
