...
export class WebVitalsMonitor {
    ...
    try {
      ...
      document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'hidden' && this.collectionState.lcp) {
          this.reportMetric(
            'LCP',
            this.collectionState.lcp,
            this.ratePerformance('LCP', this.collectionState.lcp),
            true
          )
        }
      })
        ...
        entries.forEach((entry: any) => {
          ...
          if (entry.entryType === 'layout-shift' && !entry.hadRecentInput) {
            ...
            if (this.debugMode && entry.value > 0.05) {
              console.log(
                `[WebVitalsMonitor] Layout shift detected: ${entry.value.toFixed(4)}, Total CLS: ${clsValue.toFixed(4)}`
              )
  ...
  private reportMetric(metricName: string, value: number, rating: PerformanceRating, isFinal: boolean = false): void {
    // 임계값 체크 및 알림
    if (rating === 'poor') {
    ...
    if (this.debugMode) {
      console.log(
        `[WebVitalsMonitor] ${metricName}: ${value.toFixed(2)}${this.getUnitForMetric(metricName)} (${rating})${isFinal ? ' [FINAL]' : ''}`
      )
    }
  }
  ...
  private debounce(func: (...args: any[]) => any, wait: number): (...args: any[]) => void {
