...
const projectHandlers = [
  ...
  // 팀원 초대
  rest.post('/api/projects/:id/invite', (req, res, ctx) => {
    const { id } = req.params
...
const calendarHandlers = [
  ...
  // 캘린더 이벤트 생성
  rest.post('/api/calendar/events', (req, res, ctx) => {
  ...
  // 캘린더 이벤트 업데이트 (드래그앤드롭)
  rest.put('/api/calendar/events/:id', (req, res, ctx) => {
    const { id } = req.params
    const updateData = req.body as any
...
const feedbackHandlers = [
  ...
  // 댓글 작성
  rest.post('/api/videos/:id/comments', (req, res, ctx) => {
    const { id } = req.params
  ...
  // 댓글 반응 (좋아요, 사랑 등)
  rest.post('/api/comments/:id/reactions', (req, res, ctx) => {
    const { id } = req.params
...
const errorHandlers = [
  ...
  // 403 권한 에러 시뮬레이션
  rest.get('/api/error/forbidden', (req, res, ctx) => {
    return res(
      ctx.status(403),
      ctx.json({
        success: false,
  ...
  // 404 Not Found 시뮬레이션
  rest.get('/api/error/notfound', (req, res, ctx) => {
    return res(
      ctx.status(404),
      ctx.json({
        success: false,
...
const analyticsHandlers = [
  // 프로젝트 통계
  rest.get('/api/analytics/projects', (req, res, ctx) => {
    return res(
      ctx.status(200),
      ctx.json({
        success: true,
  ...
  // 사용자 활동 통계
  rest.get('/api/analytics/activity', (req, res, ctx) => {
    return res(
      ctx.status(200),
      ctx.json({
        success: true,
