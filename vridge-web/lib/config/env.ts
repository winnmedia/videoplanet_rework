...
const validatedEnv: EnvVars = (() => {
  ...
  try {
    ...
    const skipValidation = process.env.SKIP_ENV_VALIDATION || process.env.NEXT_PUBLIC_SKIP_ENV_VALIDATION

    // Critical environment variables that should always be validated
    const criticalEnvVars = ['NEXT_PUBLIC_API_URL', 'NEXT_PUBLIC_BACKEND_URL', 'NEXT_PUBLIC_APP_URL']

    // Check if critical environment variables are present
    const criticalVarsPresent = criticalEnvVars.every(key => process.env[key] && process.env[key]?.trim().length > 0)
...
class ConfigManager {
  ...
  private getClientEnvVars(): Partial<EnvVars> {
    // For client-side, use Next.js injected environment variables with fallbacks
    // Apply the enhanced boolean parsing for client-side variables too
    const parseBoolean = (val: unknown): boolean => {
      ...
      return truthyValues.includes(lowerVal)
    }

    return {
      NEXT_PUBLIC_APP_ENV: (process.env.NEXT_PUBLIC_APP_ENV || 'development') as Environment,
  ...
  private loadConfig(): AppConfig {
    ...
    const env = typeof window !== 'undefined' ? this.getClientEnvVars() : validatedEnv

    return {
      // Application
      env: (env.NEXT_PUBLIC_APP_ENV || 'development') as Environment,
