...
import type { CalendarEvent } from '@/entities/project/model/calendar-types'
...
export default function CalendarPage() {
  ...
  const filteredEvents = useMemo(() => {
    ...
    if ((filter.selectedOrganizations || []).length > 0) {
      events = events.filter(
        event => event.project.organization && (filter.selectedOrganizations || []).includes(event.project.organization)
      )
    }
    ...
    // 프로젝트 가시성 필터
    events = events.filter(event => projectVisibility[event.project.id] !== false)
  ...
  return (
    <div className="min-h-screen bg-gray-50" role="application" aria-label="캘린더 애플리케이션">
      ...
      {/* 메인 컨텐츠 */}
      <main className="ml-[18.75rem] min-h-screen pt-20">
        <div className="container mx-auto max-w-7xl px-4 py-8">
          {/* Page Header */}
          <header className="mb-8" role="banner" aria-label="캘린더">
            <h1 data-testid="calendar-heading" className="text-3xl font-bold text-gray-900">
              전체 일정
            </h1>
            <p className="mt-2 text-gray-600">프로젝트 일정을 한눈에 확인하세요</p>
          </header>

          {/* 뷰 모드 전환 버튼 */}
          <div className="mb-6 flex items-center gap-2" role="group" aria-label="보기 모드 선택">
            <button
      ...
      {/* 라이브 리전 (스크린 리더용) */}
      <div role="status" aria-live="polite" aria-label="캘린더 상태" className="sr-only">
