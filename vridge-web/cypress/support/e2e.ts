...
declare global {
  namespace Cypress {
    interface Chainable {
      ...
      checkApiContract(response: any): Chainable<void>
      ...
      // 성능 테스트를 위한 커스텀 명령어
      measureWebVitals(): Chainable<any>
...
Cypress.Commands.add('waitForErrorDisplay', () => {
  cy.get('[data-testid="error-display"], [data-cy="error-display"]', { timeout: 10000 })
    .should('be.visible')
})
...
Cypress.Commands.add('verifyErrorRecovery', () => {
  ...
  // 재시도 버튼 클릭
  cy.get('[data-testid="retry-button"], [data-cy="retry-button"]')
    .should('be.visible')
    .click()
    
  // 에러 해결 확인
  cy.get('[data-testid="error-display"], [data-cy="error-display"]')
    .should('not.exist')
})
...
Cypress.Commands.add('checkApiContract', (response: any) => {
  // 응답 헤더 검증
...
Cypress.Commands.add('checkPerformanceThresholds', () => {
  ...
  cy.measureWebVitals().then((metrics: any) => {
