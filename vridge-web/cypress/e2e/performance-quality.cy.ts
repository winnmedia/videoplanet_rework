...
describe('Performance & Quality Gates E2E Tests', () => {
    ...
    it('Critical 리소스 로딩 시간이 임계값 이내여야 한다', () => {
      ...
      cy.window().then(win => {
        ...
        const criticalResources = resources.filter(
          resource =>
            resource.name.includes('.css') || resource.name.includes('main') || resource.name.includes('chunk')
        )
      ...
      cy.get('img').each($img => {
        ...
        if (img.src) {
          const isOptimizedFormat =
            img.src.includes('.webp') || img.src.includes('.avif') || img.src.includes('_next/image') // Next.js Image 컴포넌트
        ...
        cy.wrap($img).then($element => {
          ...
          if (naturalWidth > displayWidth * 2 || naturalHeight > displayHeight * 2) {
            cy.task(
              'log',
              `Image oversized: ${img.src} (${naturalWidth}x${naturalHeight} -> ${displayWidth}x${displayHeight})`
            )
          }
        })
      })
  ...
  describe('번들 크기 및 코드 분할 검증', () => {
    it('JavaScript 번들 크기가 임계값 이내여야 한다', () => {
      ...
      cy.window().then(win => {
        ...
        const jsResources = resources.filter(
          resource => resource.name.includes('.js') && !resource.name.includes('cypress')
        )
    ...
    it('코드 분할이 적절히 적용되어야 한다', () => {
      ...
      cy.window().then(win => {
        const resources = win.performance.getEntriesByType('resource') as PerformanceResourceTiming[]
        const jsChunks = resources.filter(resource => resource.name.includes('chunk') || resource.name.includes('lazy'))
    ...
    it('JavaScript Heap 크기가 임계값 이내여야 한다', () => {
      ...
      cy.window().then(win => {
        if ('performance' in win && 'memory' in win.performance) {
          const memory = (win.performance as unknown as { memory: { usedJSHeapSize: number; totalJSHeapSize: number } })
            .memory
    ...
    it('Event Listener 누수가 없어야 한다', () => {
      ...
      cy.window().then(win => {
        ...
        if ('getEventListeners' in win) {
          const listeners = (
            win as unknown as { getEventListeners: (el: Document) => Record<string, unknown[]> }
          ).getEventListeners(win.document)
          initialListenerCount = Object.keys(listeners).reduce((count, type) => count + listeners[type].length, 0)
      ...
      cy.window().then(win => {
        if ('getEventListeners' in win) {
          ...
          const currentListenerCount = Object.keys(listeners).reduce((count, type) => count + listeners[type].length, 0)
  ...
  describe('네트워크 최적화 검증', () => {
    it('HTTP/2 또는 HTTP/3를 사용해야 한다', () => {
      ...
      cy.window().then(win => {
        const resources = win.performance.getEntriesByType('resource') as PerformanceResourceTiming[]
        const mainResources = resources.filter(resource => resource.name.includes(win.location.origin))
    ...
    it('적절한 캐시 헤더가 설정되어야 한다', () => {
      ...
      cy.window().then(win => {
        ...
        staticResources.forEach(resource => {
          ...
          const cacheHit = resource.transferSize === 0 || (resource.transferSize > 0 && resource.transferSize < 1000)
    ...
    it('리소스 압축이 적용되어야 한다', () => {
      ...
      cy.window().then(win => {
        const resources = win.performance.getEntriesByType('resource') as PerformanceResourceTiming[]
        const textResources = resources.filter(resource => resource.name.match(/\.(css|js|html|json|svg)$/i))
        ...
        textResources.forEach(resource => {
          ...
          if (transferSize > 0 && decodedSize > 0) {
            ...
            if (decodedSize > 1024) {
              // 1KB 이상인 파일만 검사
              expect(compressionRatio).to.be.lessThan(
                0.7,
