...
describe('Collaboration Performance Tests', () => {
    ...
    it('중복 요청이 효과적으로 제거되어야 한다', () => {
      ...
      cy.window().then(win => {
        ...
        cy.then(() => {
          ...
          expect(requestCount.collaboration).to.be.lessThan(
            10,
            'Request deduplication should limit concurrent requests'
          )
  ...
  describe('Bundle Size Impact', () => {
    it('협업 기능의 번들 크기 영향이 제한되어야 한다', () => {
      ...
      cy.window().then(win => {
        ...
        const collaborationResources = resources.filter(
          resource => resource.name.includes('collaboration') || resource.name.includes('collab')
        )
    ...
    it('협업 기능이 지연 로딩되어 초기 번들에 영향을 주지 않아야 한다', () => {
      ...
      cy.window().then(win => {
        ...
        const hasCollaborationInInitial = initialResources.some(resource => resource.name.includes('collaboration'))

        expect(hasCollaborationInInitial, 'Collaboration should not be loaded on non-collaboration pages').to.be.false
      })
      ...
      cy.window().then(win => {
        ...
        const collaborationResource = projectResources.find(resource => resource.name.includes('collaboration'))
  ...
  describe('Performance Under Load', () => {
    it('다중 사용자 협업 시나리오에서 성능이 유지되어야 한다', () => {
      ...
      cy.window().then(win => {
        // 10명의 동시 사용자 시뮬레이션
        ;(win as unknown as { __mock_active_users_count: number }).__mock_active_users_count = 10
      })
